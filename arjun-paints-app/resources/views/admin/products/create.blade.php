<x-app-layout>
    <x-slot name="header">
        <h2 class="font-display text-xl font-semibold text-ink-900">{{ __('Add product') }}</h2>
    </x-slot>
    ...
    <div class="py-8">
        <div class="mx-auto max-w-2xl sm:px-6 lg:px-8">
            ...
            <form method="POST" action="{{ route('products.store') }}" class="space-y-6 rounded-lg bg-white p-6 shadow-sm sm:p-8">
                @csrf
                <div>
                    ...
                    <select id="brand_id" name="brand_id" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-brand-500 focus:ring-brand-500">
                        ...
                        @foreach ($brands as $b)
                            <option value="{{ $b->id }}" @selected(old('brand_id', $selectedBrandId) == $b->id)>{{ $b->name }}</option>
                        @endforeach
                    </select>
